...
- **Global mode 2**: all four producers accept batched `(B, N, M)` global-index neighbor matrices with aligned lattice shifts; derivative support matches the flat path above, and Hessians select real atoms. Ewald and PME estimate their splitting parameters from the real atoms of each system, so a system's energy does not depend on the padding width of its batch and matches the flat evaluation to rounding. See the calculator docs for the input contract.
