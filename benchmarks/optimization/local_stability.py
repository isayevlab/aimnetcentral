"""Optional ASE/TorchSim stability checks for optimization benchmark modes."""
...
from local_metrics import md_drift_metrics
...
def run_ase_geometry_optimization(
    *,
    ...
    calc = _make_calc(model, device, nb_threshold, coulomb, compile_model=compile_model, neighbor_skin=neighbor_skin)
...
def run_ase_nve(
    *,
    ...
    atoms.calc = AIMNet2ASE(
        _make_calc(model, device, nb_threshold, coulomb, compile_model=compile_model, neighbor_skin=neighbor_skin)
    )
...
def stability_pass(
    ...
    strict: Mapping[str, Any] | None = None,
    *,
    drift_ratio: float = 1.2,
) -> bool | None:
    if mode.get("skipped"):
        return None
