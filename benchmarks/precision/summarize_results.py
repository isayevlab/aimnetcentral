...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize AIMNet precision/performance benchmark JSONL results.")
...
def render_markdown(rows: list[dict[str, Any]]) -> str:
    ...
    non_strict = [r for r in complete if r.get("policy") != "strict"]
    strict = [r for r in complete if r.get("policy") == "strict"]
    accepted = [r for r in non_strict if accepted_row(r)]

    lines = [
        "# AIMNet Precision And Performance Summary",
        "",
        f"- Rows: {len(rows)} complete, {len(skipped)} skipped",
    ...
    if accepted:
        lines.append("Accepted non-strict policy/workload combinations:")
        lines.extend(f"- {r['workload']} / {r['policy']}: speedup {fmt_speedup(r.get('speedup'))}" for r in accepted)
    else:
        lines.append("No non-strict policy met the combined accuracy, stability, and meaningful-speedup gates.")

    if strict:
        strict_pass = sum(bool(r.get("gates", {}).get("accuracy_pass") and r.get("gates", {}).get("stability_pass")) for r in strict)
    ...
    if non_strict:
        lines.extend(["", "## Non-Strict Policy Aggregates", "", aggregate_table(non_strict)])
        lines.extend(["", "## Rejections", ""])
        for r in non_strict:
            if accepted_row(r):
                continue
            lines.append(f"- {r['workload']} / {r['policy']}: {rejection_reason(r)}")

    if skipped:
        lines.extend(["", "## Skipped", ""])
        for r in skipped:
            lines.append(f"- {r.get('workload')} / {r.get('policy')}: {r.get('reason', 'skipped')}")
...
def accepted_row(row: dict[str, Any]) -> bool:
    gates = row.get("gates", {})
    return bool(gates.get("accuracy_pass") and gates.get("stability_pass") and gates.get("meaningful_speedup"))


def matrix_table(rows: list[dict[str, Any]]) -> str:
    header = "| Workload | Policy | Median ms | Speedup | Accuracy | Stability | Meaningful |"
    sep = "| --- | --- | ---: | ---: | --- | --- | --- |"
    body = []
    for r in sorted(rows, key=lambda x: (str(x.get("workload")), str(x.get("policy")))):
        gates = r.get("gates", {})
        timing = r.get("timing", {})
        body.append(
            "| "
...
def aggregate_table(rows: list[dict[str, Any]]) -> str:
    ...
    header = "| Policy | Speedup min | Speedup median | Speedup max | Accuracy pass | Stability pass | Meaningful pass |"
    sep = "| --- | ---: | ---: | ---: | ---: | ---: | ---: |"
    body = []
    for policy, vals in sorted(groups.items()):
        speedups = [float(r["speedup"]) for r in vals if r.get("speedup") is not None]
        ...
        body.append(
            ...
            + " | ".join(
                [
                    ...
                    f"{sum(bool(g.get('stability_pass')) for g in gates)}/{len(vals)}",
...
def rejection_reason(row: dict[str, Any]) -> str:
    ...
    if not gates.get("stability_pass"):
        reasons.append("stability gate failed")
    if not gates.get("meaningful_speedup"):
