"""Precision comparison metrics for AIMNet outputs and trajectories."""
...
DEFAULT_PRECISION_GATES = PrecisionGates()
...
def energy_metrics(mode_energy: Tensor | None, strict_energy: Tensor | None, natoms: int | Tensor | None = None) -> dict[str, float | None]:
...
def charge_metrics(mode_charges: Tensor | None, strict_charges: Tensor | None, target_charge: Any) -> dict[str, float | None]:
    return {
        "charge_mae_e": tensor_mae(mode_charges, strict_charges),
        "charge_max_e": tensor_max_abs(mode_charges, strict_charges),
        "charge_sum_max_e": charge_sum_error(mode_charges, target_charge),
    }
...
def output_metrics(
    ...
    *,
    ...
    metrics.update(charge_metrics(result.get("charges"), strict.get("charges"), target_charge))
...
def accuracy_pass(metrics: dict[str, float | None], gates: PrecisionGates = DEFAULT_PRECISION_GATES) -> bool:
    checks = (
        ("energy_mae_ev", gates.energy_mae_ev),
        ("force_rmse_ev_per_a", gates.force_rmse_ev_per_a),
        ("force_max_ev_per_a", gates.force_max_ev_per_a),
        ("charge_sum_max_e", gates.charge_sum_e),
